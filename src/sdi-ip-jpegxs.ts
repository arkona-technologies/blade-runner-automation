...
export async function sdi_to_ip(vm: VAPI.VM.Any) {
  ...
  const NUM_RATIO = z.coerce
    .number()
    .gt(5)
    .lte(40)
    .int()
    .default(5)
    .parse(process.env["NUM_RATIO"]);
  ...
  const tx = await stream_video(sdi_in.video, {
    transport_format: {
      variant: "JPEG_XS",
      value: {
        add_st2110_40: ANC_DATA,
        compression: { value: { ratio: NUM_RATIO }, variant: "Ratio" },
  ...
  console.log(
    `${(await tx.v_src.status.read()).source?.raw.kwl.padEnd(
      42
    )} -> ${await tx.row_name()} `
  );
